...
| 优先级 | 关联性 | 运算符说明        | 运算符                               | 常用替代项          |
| ------ | ------ | ----------------- | ------------------------------------ | ------------------- |
| 2      | 左到右 | 成员选择          | . / ->                               | -                   |
|        |        | 数组下标          | []                                   | -                   |
|        |        | 函数调用          | ()                                   | -                   |
|        |        | 后缀递增/递减     | ++ / --                              | -                   |
| 3      | 右到左 | 计算类型/变量大小 | sizeof                               | -                   |
|        |        | 前缀递增/递减     | ++ / --                              | -                   |
|        |        | 按位取反          | ~                                    | compl               |
|        |        | 逻辑非            | !                                    | not                 |
|        |        | 一元正负          | - / +                                | -                   |
...
- `c "\\xfds"`  存储占用6个字节，\是针对反斜杠的转义
- `c"\\\x40dd"`存储占用5个字节，  \x$$ 是十六进制转义
- `c"\012"` 存储占用2个字节, $$$是八进制转义
...
```c
 ...
 int main() {
   ...
   // Hello
   // World
   // \x00 是字符串结束符，因此仅打印 "Test"
   char str[] = "Test\x00123";
   printf("%s", str); // 输出：Test
   char s1[] = "\141";   // \141 是八进制，等于十进制 97 → 字符 'a'
   char s2[] = "\14";    // 八进制 14 = 十进制 12 → 换页符（form feed, '\f'）
   char s3[] = "\777";   // 八进制 777 = 十进制 511，但 char 通常只有 8 位（0~255）
           // 实际存储为 511 % 256 = 255（即 -1 如果是有符号 char）
   char s4[] = "\123abc"; // \123 是八进制（= 83 = 'S'），后面 abc 是普通字符 → "Sabc"
   return 0;
}
```
...
#### 十三、数组指针、指针数组与二维数组传参的辨析
  ...
  5.  `int (*pf)(int,int);`  指向“参数为 (int,int)、返回 `int` 的函数”的指针。
  6. `void (*pv)(const char*);` :指向返回 `void` 的函数指针。
  7. **返回** **`int*`** **的函数:** `int *f(void)`
  8. 
...
```c
int (*p)[3] = &a;**// p 的类型是 int (*)[3]，指向整个 int[3] 数组  
int *ptr = a;  
...
- 解析：0.1存储为近似值，a*3≈0.30000004，与b=0.3的差值小于1e-6，输出“相等”；  
  若用a*3==b，输出“不相等”。
...
- - *`strncpy(dest, src, n)`* *strncpy(dest, src, n)* *strncpy(dest, src, n)* *strncpy(dest, src, n)* *：复制不超过* *`n`* *n* *n* *n*  *个字节：*
  
    - *若* *`strlen(src) >= n`* *strlen(src) >= n* *strlen(src) >= n* *strlen(src) >= n* *，则只复制前* *`n`* *n* *n* *n*  *个字节，不会在* *`dest`* *dest* *dest* *dest*  *末尾自动加* *`'\0'`* *'\0'* *'\0'* *'\0'* *（结果不是以 NUL 结束的字符串，需要手动终止：****`dest[n-1]='\0'`* *dest[n-1]='\0'* *dest[n-1]='\0'* *dest[n-1]='\0'*  *或类似处理）。*
    - *若* *`strlen(src) < n`* *strlen(src) < n* *strlen(src) < n* *strlen(src) < n* *，则复制* *`src`* *src* *src* *src* *（含* *`'\0'`* *'\0'* *'\0'* *'\0'* *）并用额外的* *`'\0'`* *'\0'* *'\0'* *'\0'*  *填充，直到共写入* *`n`* *n* *n* *n*  *字节（填充可能带来性能/语义影响，但可保证 NUL 终止）。*
...
#### 二十五、浮点型格式化输入输出（%f/%lf/%Lf/%e 与 float/double/long double）

【选择题示例】 下列关于浮点型输入输出的代码中，错误的是____A_____。


<P>
  A) `float f; scanf("%lf", &f); printf("%f", f);`
  B) `double d; scanf("%lf", &d); printf("%f", d);`
  C) `long double ld = 3.14L; printf("%Lf", ld);`
  D) `float f = 3.14f; printf("%f", f);`
<p>
  答案：A 错误。原因：scanf 中 %lf 匹配 double* 类型，但 f 是 float 类型，格式符与变量类型不匹配，会导致内存解析错误；printf 中虽 %f 可输出 float，但输入阶段已造成数据乱码。


【printf 核心规则（值传递+隐式提升）】

- 底层逻辑：printf 接收的是「值」，float 类型参数传入时会自动提升为 double，因此 %f 可兼容 float 和 double 输出。
- 格式符匹配规则：

  | 格式符 | 匹配类型     | 说明                                        |
  | ------ | ------------ | ------------------------------------------- |
  | %f     | double/float | 输出十进制浮点数（float 自动提升为 double） |
  | %lf    | double       | 与 %f 等价（仅为兼容性设计，无功能差异）    |
  | %Lf    | long double  | 必须用 %Lf 输出 long double（无隐式提升）   |
...
- 验证示例：
  ...
  int main() {
      ...
      double d = 3.14;
      // 以下两行输出结果完全一致！
      printf("float via %f: %f\n", f);  // f被提升为double后传入
      printf("double via %f: %f\n", d); // d直接以double传入
      return 0;
  }
  ```

- 关键结论：printf 中，%f 是「处理 double 类型」的格式符，但因 float 会自动提升为 double，所以 printf("%f", float变量) 是安全的；输出 long double 必须用 %Lf，否则格式不匹配会输出乱码。

【scanf 核心规则（地址传递+严格匹配）】

- 底层逻辑：scanf 接收的是「地址」，需根据地址指向的变量类型精准解析二进制格式，无隐式提升，错配会导致数据乱码/程序崩溃。
- 格式符匹配规则：

  | 格式符 | 匹配类型      | 说明                                             |
  | ------ | ------------- | ------------------------------------------------ |
  | %f     | float *       | 仅用于读取 float 类型（传入 float 变量的地址）   |
  | %lf    | double *      | 仅用于读取 double 类型（传入 double 变量的地址） |
  | %Lf    | long double * | 仅用于读取 long double 类型                      |

- 关键结论：scanf 中，%f ≠ %lf，必须严格匹配变量类型；%Lf 是 long double 输入的专属格式符。
...
int main() {
    ...
    scanf("%f", &f);    // %f ↔ float*
    printf("输入double: ");
    scanf("%lf", &d);   // %lf ↔ double*
    printf("输入long double: ");
    scanf("%Lf", &ld);  // %Lf ↔ long double*

    // ==== printf 输出（宽松/严格）====
    printf("float输出: %f\n", f);        // %f ↔ float（自动提升为double）
    printf("double输出: %f 或 %lf\n", d, d); // %f/%lf 均可
    printf("long double输出: %Lf\n", ld); // 必须用%Lf
...
- 正确用法：
  ...
  - float 输入用 %f，输出用 %f；
  - double 输入用 %lf，输出用 %f/%lf 均可；
  - long double 输入用 %Lf，输出用 %Lf，且常量需加 L 后缀（如 3.14L）。

- 避坑建议：

  - 禁止 scanf 中用 %lf 读取 float、用 %f 读取 double；
  - 输出 long double 时，必须加 L 前缀（%Lf），不可用 %f/%lf；
  - 浮点常量声明：float 加 f 后缀（3.14f），避免 double 隐式转换丢失精度。


【注意】

- printf 中 %lf 是 %f 的别名，仅为兼容早期编译器设计，实际使用中优先用 %f 输出 double/float；
- scanf 的格式符匹配是「内存级」的严格匹配，错配会直接破坏内存布局，而非简单的输出错误；
- long double 无隐式提升特性，输入输出必须全程使用 %Lf，且不同平台（gcc/MSVC）的存储字节数不同，但格式符规则统一。
